...
async def ultroid(event):
    ...
    if str(event.sender_id) not in owner_and_sudos():
        ...
        if not udB.get("STARTMSG"):
            ...
            await event.reply(
                f"Ne Ne !! ( ´ ω ` )ノﾞ [{get_display_name(u)}](tg://user?id={u.id}), this is Suisei Assistant of [{ultroid_bot.me.first_name}](tg://user?id={ultroid_bot.uid})!\n\n{ok}",
